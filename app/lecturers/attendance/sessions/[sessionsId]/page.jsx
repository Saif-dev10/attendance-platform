...
import AuditTrail from "../../components/session-record/AuditTrail";
...
export default function SessionRecordPage({ params }) {
  const record = useSessionRecord(params.sessionId);
  ...
  const handleExport = (format) => window.alert(`Export as ${format} is not wired up yet.`);

  const getRowActions = (row) => [
    { label: "Correct record", onSelect: () => record.openCorrection(row.id) },
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-charcoal">Session Record</h1>
            {record.session && (
              <p className="text-sm text-graphite-soft">
                {record.session.courseCode} — {record.session.courseTitle} · {formatDate(record.session.date)} ·{" "}
                {record.session.room}
              </p>
            )}
          </div>
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => handleExport("CSV")} className={secondaryButton}>
            <Download aria-hidden="true" className="h-3.5 w-3.5" />
            Export CSV
          </button>
          <button type="button" onClick={() => handleExport("PDF")} className={secondaryButton}>
      ...
      {!record.loading && !record.session && !record.error ? (
        ...
        <>
          <SessionStatistics summary={record.summary} loading={record.loading} />

          <div className="rounded-md border border-line bg-paper">
            <SessionRecordTabs active={tab} onChange={setTab} exceptionCount={record.exceptions.length} />
            ...
            {tab === "details" && record.session && (
              <dl className="grid grid-cols-2 gap-4 p-4 text-sm sm:grid-cols-3">
                <div>
                  <dt className="text-xs text-graphite-soft">Course</dt>
                  <dd className="text-charcoal">
                    {record.session.courseCode} — {record.session.courseTitle}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs text-graphite-soft">Room</dt>
                  <dd className="text-charcoal">{record.session.room}</dd>
                </div>
                <div>
                  <dt className="text-xs text-graphite-soft">Date</dt>
                  <dd className="text-charcoal">{formatDate(record.session.date)}</dd>
                </div>
                <div>
                  <dt className="text-xs text-graphite-soft">Time</dt>
                  <dd className="text-charcoal">{record.session.scheduledTime}</dd>
                </div>
                <div>
                  <dt className="text-xs text-graphite-soft">Level</dt>
                  <dd className="text-charcoal">{record.session.level} Level</dd>
                </div>
                <div>
                  <dt className="text-xs text-graphite-soft">Status</dt>
                  <dd className="capitalize text-charcoal">{record.session.status}</dd>
                </div>
              </dl>
            )}
          </div>
